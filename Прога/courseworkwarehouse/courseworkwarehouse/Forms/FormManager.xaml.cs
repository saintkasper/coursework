...
namespace courseworkwarehouse.Forms
{
    ...
    public partial class FormManager : Window
    {
        ...
        public void LoadDataInvoice()
        {
            //DataBank.sqlConnection.Open();
            DataTable table = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter($"Select Product_article as [Артикул товара], Title as [Название], Cost as [Цена], Quantity as [Количество], Photo as [Фото] From Product WHERE Title Like '%{tbSearchReport.Text}%' {AscDesc}", DataBank.sqlConnection);
            adapter.Fill(table);
            ...
            try
            {
                if (dgReportKorzina.ItemsSource == null)
                {
                    ...
                    if (quantityInSclad > 0)
                    {
                        ...
                        DataTable table = new DataTable();
                        SqlDataAdapter adapter = new SqlDataAdapter($"Select Product_article as [Артикул товара], Title as [Название], Cost as [Цена], Quantity as [Количество], Photo as [Фото] From Product WHERE Product_article = '{pArticle}'", DataBank.sqlConnection);
                        adapter.Fill(table);
        ...
        private void btnPrint_Click(object sender, RoutedEventArgs e)
        {
            if (dgReportKorzina.ItemsSource != null)
            {
                ...
                SqlDataAdapter adapterMax = new SqlDataAdapter("SELECT MAX([Tabular part of the receipt].Receipt_invoice_num) as [Код] FROM[Tabular part of the receipt]", DataBank.sqlConnection);
                adapterMax.Fill(max);
                ...
                for (int i = 0; i < Korzina.Rows.Count; i++)
                {
                    if (receiptnum == true)
                    {
                        adapter = new SqlCommand($"Insert Into [Tabular part of the receipt] (Receipt_invoice_num, Product_article, Quantity, Wholesale_price, Cost, Employee_num, Date_of_the_receipt) Values ('{int.Parse(max.Rows[0][0].ToString()) + 1}', '{Korzina.Rows[i][0].ToString()}','{Korzina.Rows[i][3].ToString()}', '{Korzina.Rows[i][4].ToString()}', '{Korzina.Rows[i][5].ToString()}', '{DataBank.employeenum}', '{DateTime.Today.ToShortDateString()}')", DataBank.sqlConnection);
                        adapter.ExecuteNonQuery();
                        ProdArticle = new SqlDataAdapter("SELECT [Tabular part of the receipt].Tabular_part_of_the_receipt_num, [Tabular part of the receipt].Product_article FROM [Tabular part of the receipt]", DataBank.sqlConnection);
                        ProdArticle.Fill(dtPA);
                        sqlDataAdapter = new SqlDataAdapter($"Select Quantity From Product Where Product_article = '{dtPA.Rows[dtPA.Rows.Count-1][1].ToString()}'", DataBank.sqlConnection);
                        sqlDataAdapter.Fill(dataQuantityInSklad);
                        SqlCommand Update = new SqlCommand($"Update Product Set Quantity = '{(int.Parse(dataQuantityInSklad.Rows[i][0].ToString())) - (int.Parse(Korzina.Rows[i][3].ToString()))}' Where Product_article = '{Korzina.Rows[i][0].ToString()}'", DataBank.sqlConnection);
                ...
                //SqlDataAdapter common = new SqlDataAdapter("Select TP_invoice_num From [Receipt invoice]", DataBank.sqlConnection);
                //common.Fill(dt);
                //int TPreceipt = 1;
                //try
                //{
                ...
                //for (int i = 0; i < Korzina.Rows.Count; i++)
                //{
                //    SqlCommand command = new SqlCommand($"Insert into [Receipt invoice] (TP_invoice_num, Product_article, Date_of_the_receipt) Values ('{TPreceipt}', '{Korzina.Rows[i][0].ToString()}', '{DateTime.Today.ToShortDateString()}')", DataBank.sqlConnection);
                //    command.ExecuteNonQuery();
                //}
                //DataTable dt2 = new DataTable();
                //SqlDataAdapter common2 = new SqlDataAdapter("Select Receipt_invoice_num From [Receipt invoice]", DataBank.sqlConnection);
                //common2.Fill(dt2);
                ...
                //    SqlCommand command2 = new SqlCommand($"Insert into [Tabular part of the receipt] (Receipt_invoice_num, Quantity, Cost, Wholesale_price, Employee_num) Values ('{receiptNum}', '{Korzina.Rows[i][3].ToString()}', '{Korzina.Rows[i][2].ToString()}', '{Korzina.Rows[i][4].ToString()}', '{DataBank.employeenum}')", DataBank.sqlConnection);
